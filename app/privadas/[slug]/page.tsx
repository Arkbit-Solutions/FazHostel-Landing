...
const cat = categoryPages.privadas;

export function generateStaticParams() {
  return getRoomsByCategory("privadas").map((r) => ({ slug: r.slug }));
}
...
export default async function PrivadaDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const room = getRoomBySlug(slug);
  if (!room || room.category !== "privadas") notFound();
