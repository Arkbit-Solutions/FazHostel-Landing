...
const cat = categoryPages.habitaciones;

export function generateStaticParams() {
  return getRoomsByCategory("habitaciones").map((r) => ({ slug: r.slug }));
}
...
export default async function HabitacionDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const room = getRoomBySlug(slug);
  if (!room || room.category !== "habitaciones") notFound();
